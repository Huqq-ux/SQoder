...
export function CanvasPanel() {
  ...
  return (
    <div
      className={`${
        canvasOpen ? 'w-96' : 'w-0'
      } border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shrink-0 overflow-hidden transition-all duration-200`}
    >
      <div className="w-96 p-4 h-full overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">画布</h3>
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
        ...
        {canvasContent ? (
          <div className="space-y-4">
            {canvasContent.type === 'code' && (
              <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                ...
                <pre className="p-3 text-[11px] leading-relaxed overflow-x-auto text-slate-600 dark:text-slate-400">
            ...
            {canvasContent.type === 'tool' && (
              <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
                ...
                <div className="text-[11px] text-slate-600 dark:text-slate-500 space-y-1">
                  {Object.entries(canvasContent.data || {}).map(([k, v]) => (
                    <div key={k} className="flex justify-between">
                      <span>{k}</span>
                      <span className="text-slate-800 dark:text-slate-400">{String(v)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-600 text-center mt-12">暂无内容</p>
        )}
      </div>
    </div>
  )
